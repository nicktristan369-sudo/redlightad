...
import { Lock, CheckCircle, AlertCircle, X, Coins } from "lucide-react";
...
export default function SocialLinksSection({ listingId, socialLinks, isPremium, isOwnListing }: Props) {
  ...
  const [unlockModal, setUnlockModal] = useState<{ platform: SocialPlatform; price: number } | null>(null);
          ...
          {platforms.map(([platform, cfg]) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (canClick) {
                    ...
                    setUnlockModal({ platform, price: cfg.price_coins || 0 });
                  }
                }}
                disabled={isBusy}
                className="relative flex items-center gap-3 px-4 py-3.5 rounded-lg transition-all duration-200 hover:bg-gray-50 disabled:opacity-60"
                style={{
                  background: "#FAFAFA",
              ...
              >
                ...
                {/* Label */}
                <span className="text-[13px] font-medium text-gray-800 flex-1 text-left truncate">
                  {meta.label}
                </span>
          ...
          >
            ...
            <div className="flex justify-center mb-4">
              <div className="w-16 h-16 rounded-full bg-gray-100 flex items-center justify-center">
                {Icons[unlockModal.platform]}
              </div>
            </div>

            {/* Title */}
            <h3 className="text-lg font-bold text-gray-900 text-center mb-2">
              Unlock {PLATFORM_META[unlockModal.platform].label}
            </h3>

            {/* Price */}
            <div className="flex items-center justify-center gap-2 mb-6">
              <Coins size={20} className="text-yellow-500" />
              <span className="text-2xl font-bold text-gray-900">{unlockModal.price}</span>
              <span className="text-gray-500">coins</span>
            </div>

            {/* Description */}
            <p className="text-sm text-gray-500 text-center mb-6">
              Spend {unlockModal.price} coins to unlock this {PLATFORM_META[unlockModal.platform].label} contact.
            </p>
            ...
            <div className="flex gap-3">
              <button
                onClick={() => setUnlockModal(null)}
                className="flex-1 py-3 px-4 rounded-xl text-[14px] font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={() => handleUnlock(unlockModal.platform, unlockModal.price)}
                className="flex-1 py-3 px-4 rounded-xl text-[14px] font-semibold text-white bg-gray-900 hover:bg-black transition-colors"
              >
                Unlock
              </button>
            </div>
