...
import { Phone, Star, MessageCircle, Flag, X, Send } from "lucide-react"
...
function ContactModal({ phone, whatsapp, onClose }: { phone: string | null; whatsapp: string | null; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/50" onClick={onClose}>
      <div
        className="w-full max-w-lg bg-white rounded-t-2xl p-6 pb-8 space-y-3"
...
// ── Message Modal ─────────────────────────────────────────────────────────────
function MessageModal({ listingTitle, onClose }: { listingTitle: string; onClose: () => void }) {
  const [msg, setMsg] = useState("")
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/50" onClick={onClose}>
      <div className="w-full max-w-lg bg-white rounded-t-2xl p-6 pb-8" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
        ...
        <p className="text-[13px] text-gray-400 mb-3">Til: <strong className="text-gray-700">{listingTitle}</strong></p>
        <textarea
          rows={4}
          value={msg}
          onChange={e => setMsg(e.target.value)}
          placeholder="Skriv din besked…"
          className="w-full rounded-xl border border-gray-200 px-4 py-3 text-[14px] resize-none focus:outline-none focus:border-gray-400 mb-3"
        />
        <button
          onClick={() => { alert("Besked sendt (integration pending)"); onClose(); }}
          disabled={!msg.trim()}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-[14px] font-semibold text-white disabled:opacity-40 transition-opacity"
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/50" onClick={onClose}>
      <div className="w-full max-w-lg bg-white rounded-t-2xl p-6 pb-8" onClick={e => e.stopPropagation()}>
        ...
        {sent ? (
          <p className="text-[14px] text-green-600 font-semibold text-center py-4">Tak — rapport modtaget.</p>
        ) : (
          <>
            <textarea
              rows={4}
...
// ── Main component ────────────────────────────────────────────────────────────
type Modal = "contact" | "message" | "report" | null

export default function StickyActionBar({ phone, whatsapp, listingId, listingTitle }: Props) {
  ...
  return (
    <>
        ...
        <div className="flex items-stretch divide-x divide-gray-100">
          {buttons.map((btn, i) => (
            <button
              ...
              style={{
                background: btn.primary ? "#DC2626" : "#fff",
                color: btn.primary ? "#fff" : "#6B7280",
              }}
            >
              {btn.icon}
      ...
      {modal === "message" && (
        <MessageModal listingTitle={listingTitle} onClose={() => setModal(null)} />
      )}
      {modal === "report" && (
        <ReportModal listingId={listingId} onClose={() => setModal(null)} />
      )}
    </>
  )
}
