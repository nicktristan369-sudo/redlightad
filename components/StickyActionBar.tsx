...
// ── Contact Modal ─────────────────────────────────────────────────────────────
function ContactModal({ phone, whatsapp, profileImage, name, onClose }: {
  phone: string | null; whatsapp: string | null; profileImage?: string | null; name?: string; onClose: () => void
}) {
  return (
    <Sheet onClose={onClose}>
      <div className="px-6 pt-4 pb-6">
        {/* Header med profilbillede + tekst */}
        ...
        <div className="space-y-3">
          {phone && (
            <a href={`tel:${phone}`}
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-[15px] font-semibold text-white bg-green-600">
              <Phone size={18} /> Ring: {phone}
            </a>
          )}
          {phone && (
            <a href={`sms:${phone.replace(/\s/g, "")}`}
              className="flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-[14px] font-semibold text-gray-800 border border-gray-200">
              <MessageSquare size={18} color="#374151" /> SMS: {phone}
            </a>
          )}
          {whatsapp && (
          ...
          <button onClick={onClose} className="w-full py-3 rounded-xl text-[14px] font-semibold text-gray-600 bg-gray-100">
            Luk
          </button>
        </div>
      </div>
