...
export default function BoostPage() {
  ...
  const handleBuy = async (packageId: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 402) {
          showToast("error", "Not enough Red Coins.")
        } else {
      ...
      showToast("success", premiumPkg
        ? "Activated! Your profile is now Premium VIP"
        : "Activated! Your profile is now Boosted to the top"
      )
    } catch {
      ...
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {loading ? (
          ...
          <>
            ...
            {listings.length > 1 && (
              <div>
                <label className="text-sm font-medium text-gray-700 mb-1 block">Select listing</label>
                <select
            ...
            <div className="flex gap-3 flex-wrap">
              <span className={`text-xs font-semibold px-3 py-1.5 rounded-full ${isPremiumActive ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-500"}`}>
                {isPremiumActive ? `VIP until ${formatDate(activeListing!.premium_until!)}` : "Standard profile"}
              </span>
              {isBoostActive && (
            ...
            <section>
              ...
              <p className="text-gray-500 text-sm mb-5">Premium profiles are automatically shown at the top of the front page</p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {PREMIUM_PACKAGES.map(pkg => (
                  <div key={pkg.id} className={`relative bg-white rounded-2xl border-2 p-5 flex flex-col items-center text-center transition-all hover:shadow-md ${pkg.popular ? "border-red-400 shadow-md" : "border-gray-100"}`}>
                    {pkg.popular && (
                      <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-red-500 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wide">Most Popular</span>
                    )}
                    {pkg.discount > 0 && (
                    ...
                    >
                      {buyingId === pkg.id ? (
                        ...
                        `Buy for ${pkg.coins.toLocaleString()} coins`
                      )}
                    </button>
                  </div>
                ))}
            ...
            <section>
              ...
              <p className="text-gray-500 text-sm mb-5">Jump to the absolute top — above all premium profiles</p>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white rounded-2xl border border-gray-100 p-5 text-center">
                  ...
                  <p className="font-bold text-gray-900 mb-1">Free profile</p>
                  <p className="text-gray-500 text-sm">Shown normally in the list</p>
                </div>
                <div className="bg-white rounded-2xl border border-yellow-200 p-5 text-center">
                  <div className="text-3xl mb-3">&#x2B50;</div>
                  <p className="font-bold text-gray-900 mb-1">Premium profile</p>
                  <p className="text-gray-500 text-sm">Shown at the top, VIP badge</p>
                </div>
                <div className="bg-white rounded-2xl border border-red-200 p-5 text-center">
                  ...
                  <p className="text-gray-500 text-sm">Absolute #1 position, temporary</p>
                </div>
              </div>
            </section>
          </>
