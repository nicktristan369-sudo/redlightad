...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from("cam_tip_menu").insert({
...
export async function DELETE(req: NextRequest) {
  ...
  const { data: item } = await supabaseAdmin.from("cam_tip_menu").select("listing_id").eq("id", id).single()
  if (!item) return NextResponse.json({ error: "Not found" }, { status: 404 })

  const { data: listing } = await supabaseAdmin.from("listings").select("user_id").eq("id", item.listing_id).single()
  if (!listing || listing.user_id !== user.id) return NextResponse.json({ error: "Not your listing" }, { status: 403 })

  await supabaseAdmin.from("cam_tip_menu").delete().eq("id", id)
