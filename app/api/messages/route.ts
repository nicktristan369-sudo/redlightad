...
export async function POST(req: NextRequest) {
  try {
    ...
    if (authErr || !user) {
      console.error("[Messages] Auth error:", authErr)
    ...
    if (listingErr || !listing) {
      console.error("[Messages] Listing error:", listingErr)
    ...
    const { data: existing, error: convCheckErr } = await db
      .from("conversations")
    ...
    if (convCheckErr) {
      console.error("[Messages] Conversation check error:", convCheckErr)
      return NextResponse.json({ error: "Database error: " + convCheckErr.message }, { status: 500 })
    }

    if (existing) {
      conversationId = existing.id
      // Opdater last_message + unread
      const { error: updateErr } = await db.from("conversations").update({
        last_message: content.trim(),
        last_message_at: new Date().toISOString(),
        provider_unread: (existing.provider_unread ?? 0) + 1,
      }).eq("id", conversationId)
      if (updateErr) {
        console.error("[Messages] Conversation update error:", updateErr)
      ...
      if (convErr || !newConv) {
        console.error("[Messages] Conversation insert error:", convErr)
    ...
    if (msgErr) {
      console.error("[Messages] Message insert error:", msgErr)
