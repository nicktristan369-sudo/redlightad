          ...
          {showMenu && (
            <>
              ...
              <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-xl shadow-xl z-50 py-1 overflow-hidden">
                ...
                <button onClick={() => { setShowMenu(false); clearHistory(); }} className="flex items-center gap-2 w-full text-left px-4 py-2.5 hover:bg-gray-50 text-sm text-gray-700">🗑️ Clear history</button>
                ...
                <div className={`max-w-[75%] sm:max-w-[65%] px-3 py-2 ${isMe ? 'bg-red-500 text-white rounded-2xl rounded-br-md' : 'bg-white text-gray-900 rounded-2xl rounded-bl-md shadow-sm'}`}>
                  {msg.image_url && (
                    <img
                      src={msg.image_url}
                      alt="attachment"
                      className="rounded-lg cursor-pointer mb-1.5 hover:opacity-90 transition"
                  ...
                  <div className={`text-[10px] flex items-center justify-end gap-1 mt-0.5 ${isMe ? 'text-red-200' : 'text-gray-400'}`}>
                    ...
                    {isMe && (
                      <span style={{ color: msg.read_at ? (isMe ? '#FDE8E8' : '#3B82F6') : (isMe ? 'rgba(255,255,255,0.5)' : '#9CA3AF') }}>{msg.read_at ? '✓✓' : '✓'}</span>
                    )}
                  </div>
                </div>
              )}
