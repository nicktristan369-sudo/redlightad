...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const f = async () => { try { const s=createClient(); const{data:conv}=await s.from('conversations').select('*').eq('id',conversationId).single(); if(!conv){setLoading(false);return;} const otherId=currentUserId===conv.provider_id?conv.customer_id:conv.provider_id; const{data:profile}=await s.from('profiles').select('id,full_name,avatar_url,email,username').eq('id',otherId).maybeSingle(); let dn=profile?.full_name||profile?.username||profile?.email||'User'; if(otherId===conv.provider_id){const{data:l}=await s.from('listings').select('display_name').eq('user_id',otherId).limit(1).maybeSingle(); if(l?.display_name) dn=l.display_name;} setOtherUser({id:otherId,display_name:dn,avatar_url:profile?.avatar_url||null}); } catch(e){console.error(e);} setLoading(false); }; f();
