...
export default function PersonalLinkPage() {
  ...
  const allSocialLinks = [
    hasInstagram && {
      ...
      label: "Instagram",
      bg: "#1a1a1a",
      icon: <img src="/logos/instagram.jpg" alt="Instagram" style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 6 }} />,
    },
    hasOnlyFans && {
      href: socialLinks.onlyfans?.url || `https://onlyfans.com/${hasOnlyFansName}`,
      label: "OnlyFans",
      bg: "#1a1a1a",
      icon: <img src="/logos/onlyfans.jpg" alt="OnlyFans" style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 6 }} />,
    },
    hasSnapchat && {
      href: socialLinks.snapchat?.url || `https://snapchat.com/add/${socialLinks.snapchat?.username}`,
      label: "Snapchat",
      bg: "#1a1a1a",
      icon: <img src="/logos/snapchat.jpg" alt="Snapchat" style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 6 }} />,
    },
    hasX && {
      href: socialLinks.x?.url || socialLinks.twitter?.url || `https://x.com/${socialLinks.x?.username}`,
      label: "X / Twitter",
      bg: "#1a1a1a",
      icon: <img src="/logos/x.jpg" alt="X" style={{ width: 28, height: 28, objectFit: "cover", borderRadius: 6 }} />,
    },
    hasViber && {
      href: socialLinks.viber?.url || `viber://chat?number=${listing.phone}`,
      label: "Viber",
      bg: "#7360f2",
      icon: <span style={{ fontWeight: 900, fontSize: 14, color: "white" }}>V</span>,
    },
    hasLine && {
      href: socialLinks.line_app?.url || `https://line.me/ti/p/${socialLinks.line_app?.username}`,
      label: "LINE",
      bg: "#00B900",
      icon: <span style={{ fontWeight: 900, fontSize: 12, color: "white" }}>LINE</span>,
    },
    hasWechat && {
      href: socialLinks.wechat?.url || `weixin://dl/chat?${socialLinks.wechat?.username}`,
      label: "WeChat",
      bg: "#07C160",
      icon: <span style={{ fontWeight: 900, fontSize: 11, color: "white" }}>WeChat</span>,
    },
            ...
            <div className="grid grid-cols-2 gap-2">
              {allSocialLinks.map(link => (
                <a
                  ...
                  href={link.href}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-3 rounded-2xl px-3 py-3 hover:opacity-80 transition-opacity"
